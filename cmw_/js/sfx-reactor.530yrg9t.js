...
/*
	setTimeout(() => {
		...
		function parallax(e) {
			...
			let _depth = `${50 - (_mouseX - _w) * -0.01}% ${50 - (_mouseY - _h) * -0.1}%`;
			document.querySelector(".page-header__championship-banner").style.backgroundPosition = _depth;
		}
...
const squares = document.getElementsByClassName('ballon');
...
function translateSquare(sq_el) {
  ...
  sq_el.style.transform = 'translate3d(' + dx + 'px, ' + dy + '%,' + dz + 'px)';
}
...
initSquares();
	...
	function checkDevice() { 
		...
		return true, // mobile
		setTimeout(() => {document.querySelector(".page-header__championship-banner").style = "transition: .5s";}, 1000);
		// setTimeout(() => {document.querySelector(".ballons_container").style.filter = "blur(.15vh)";}, 01);
	}
	else {
		return false; // no mobile
	}
	}
	console.log(checkDevice()); // false = no mobile
...
function blowBubble() {
	var bubble = [];
	bubble[0] = parseInt(Math.random()*7.5+3.75); // radius and size
	bubble[1] = parseInt(Math.random()*canvas.width); // x
	bubble[2] = canvas.height+bubble[0]*2; // y
	bubble[3] = Math.random()*1.75+1.5; // y speed
	return bubble;
}
...
window.requestAnimFrame =
    ...
    function (moveBubbles) {
        window.setTimeout(moveBubbles, 100 / 60);
};
